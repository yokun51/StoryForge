...
export const installationSchema = z.object({
	...
	startParams: z.string().trim().catch(""),
	...
	const { mutateAsync: initializeGame, isPending: initializePending } =
		useMutation({
			mutationFn: ({
				...
				invoke("initialize_game", { clientSettings, path }) as Promise<string>,
			onError: (error, variables) => {
				toast.error(`Error initializing game: ${error}`, {
					id: `initialize-game-${variables.path}`,
				});
			},
			onMutate: (variables) => {
	...
	const form = useForm({
		...
		onSubmit: async ({ value }) => {
			...
			if (useLocalProfile && actualVersionToInstall === BUNDLED_LOCAL_VERSION) {
				...
				clientSettings = JSON.stringify(clientSettingsObj, null, 2);
			}
			...
			await initializeGame({ clientSettings, path: value.path.trim() });
		...
		>
			...
			<DialogContent>
				...
				<div className="space-y-5">
								...
								<div className="grid gap-2">
									...
									<Input
										...
										onChange={(e) => {
											...
											if (trimmedName.length > 0 && appFolder) {
												const safeName = makeStringFolderSafe(trimmedName);
												form.setFieldValue(
													"path",
													buildInstallationPath(
														installationsParent ?? appFolder,
					...
					<Button
						className="w-full"
						disabled={form.state.isSubmitting || isPending || initializePending}
						onClick={() => form.handleSubmit()}
						type="button"
					>
						{form.state.isSubmitting || isPending || initializePending
							? "Adding..."
							: "Add Installation"}
					</Button>
				</div>
